...
- `"homeassistant"` - publish Home Assistant auto-discovery messages to the
  `homeassistant/...` topics. The default value is `true`. Set to `false` to
  disable auto-discovery completely while keeping device state and availability
  publications on `base_topic`.
